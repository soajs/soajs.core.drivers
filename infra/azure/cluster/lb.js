...
const lbs = {
				...
				async.auto({listLoadBalancers, listPublicIps}, function (error, results) {
					utils.checkError(error, 732, cb, () => {
						async.map(results.listLoadBalancers.loadBalancers, function (oneloadBalancer, callback) {
							return callback(null, helper.buildLoadBalancerRecord({
								loadBalancer: oneloadBalancer,
								publicIpsList: results.listPublicIps.ipAddresses
							}));
			...
			utils.checkError(error, 700, cb, () => {
					...
					if (options.params.rules && Array.isArray(options.params.rules) && options.params.rules.length > 0) {
						options.params.rules.forEach((oneRule) => {
							...
							if (oneRule.ports && Array.isArray(oneRule.ports) && oneRule.ports.length > 0) {
								oneRule.ports.map((onePort) => {
									onePort.ipConfigName = oneRule.name;
								});
							...
							if (oneRule.natRules && Array.isArray(oneRule.natRules) && oneRule.natRules.length > 0) {
								oneRule.natRules.map((oneNatRule) => {
									oneNatRule.ipConfigName = oneRule.name;
								});
							...
							if (oneRule.natPools && Array.isArray(oneRule.natPools) && oneRule.natPools.length > 0) {
								oneRule.natPools.map((oneNatPool) => {
									oneNatPool.ipConfigName = oneRule.name;
								});
				...
				function applyLbConfig(result, callback) {
					...
					let params = {
						...
						frontendIPConfigurations: lbs.computeFrontendIpConfigs(options.params.ipConfigs, {
							subscriptionId: options.infra.api.subscriptionId,
				...
				function applyLbRules(result, callback) {
					...
					let portsConfig = lbs.buildLoadBalancerRules(options.params.ports, {
						subscriptionId: options.infra.api.subscriptionId,
						group: options.params.group,
						lbName: options.params.name
					});
					let params = {
						location: options.params.region,
					...
					if (options.params.natPools && Array.isArray(options.params.natPools) && options.params.natPools.length > 0) {
						params.inboundNatPools = lbs.buildInboundNATPools(options.params.natPools, {
							subscriptionId: options.infra.api.subscriptionId,
							group: options.params.group,
							lbName: options.params.name
						});
					} else if (options.params.natRules && Array.isArray(options.params.natRules) && options.params.natRules.length > 0) {
						params.inboundNatRules = lbs.buildInboundNATRules(options.params.natRules, {
							subscriptionId: options.infra.api.subscriptionId,
							group: options.params.group,
							lbName: options.params.name
						});
	...
	delete: function (options, cb) {
		...
		driverUtils.authenticate(options, (error, authData) => {
			utils.checkError(error, 700, cb, () => {
				...
				networkClient.loadBalancers.deleteMethod(options.params.group, options.params.name, function (error) {
		...
		if (Array.isArray(ports) && ports.length > 0) {
			ports.forEach(onePort => {
				let oneRule = {
					name: onePort.name,
					backendAddressPool: {
						id: helper.buildAzureId(config.lbAddressPoolIdFormat, {
							SUBSCRIPTION_ID: options.subscriptionId,
							GROUP_NAME: options.group,
							LB_NAME: options.lbName,
							ADDRESS_POOL_NAME: onePort.addressPoolName
						})
					},
					...
					frontendIPConfiguration: {
						id: helper.buildAzureId(config.lbIpConfigIdFormat, {
							SUBSCRIPTION_ID: options.subscriptionId,
							GROUP_NAME: options.group,
							LB_NAME: options.lbName,
							CONFIG_NAME: onePort.ipConfigName
						})
					},
					probe: {id: ''}
				};
				...
				oneRule.probe.id = helper.buildAzureId(config.lbHealthProbeIdFormat, {
					SUBSCRIPTION_ID: options.subscriptionId,
					GROUP_NAME: options.group,
					LB_NAME: options.lbName,
					LB_PROBE_NAME: oneProbe.name
				});
	 ...
	 * @return {Array}
	 */
	computeFrontendIpConfigs: function (ipConfigs) {
		let configs = [];
				...
				natPoolsOutput.push({
					...
					frontendIPConfiguration: {
						id: helper.buildAzureId(config.lbIpConfigIdFormat, {
							...
							GROUP_NAME: options.group,
							LB_NAME: options.lbName,
							CONFIG_NAME: oneNatPool.ipConfigName
						})
					}
				});
			});
				...
				inboundNatRulesOutput.push({
					...
					frontendIPConfiguration: {
						id: helper.buildAzureId(config.lbIpConfigIdFormat, {
							...
							GROUP_NAME: options.group,
							LB_NAME: options.lbName,
							CONFIG_NAME: oneNatRule.ipConfigName
						})
					}
				});
			});
		...
		if (lbRecord && lbRecord.frontendIPConfigurations && Array.isArray(lbRecord.frontendIPConfigurations) && lbRecord.frontendIPConfigurations.length > 0) {
			if (options.params && options.params.rules && Array.isArray(options.params.rules) && options.params.rules.length > 0) {
				...
				lbRecord.frontendIPConfigurations.forEach((oneIpConfig) => {
					...
					if (!foundRule) {
						detectDeletedRules = true;
					}
				});
				
				if (detectDeletedRules) {
					reverseUpdate = true;
				}
			} else {
				reverseUpdate = true;
