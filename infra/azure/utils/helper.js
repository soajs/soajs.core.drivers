...
const helper = {
	...
	buildVMRecord: function (opts) {
		...
		if (opts.vm) {
			...
			if (opts.vm.name) {
				record.name = opts.vm.name;
			}
			if (opts.vm.name) {
				record.id = opts.vm.name;
			}
			
			record.labels = {};
			if (opts.vm.tags) {
				record.labels = opts.vm.tags;
			}
			if (opts.vm.location) {
				record.labels['soajs.service.vm.location'] = opts.vm.location;
			}
			if (opts.vm.id) {
				let idInfo = opts.vm.id.split('/');
				record.labels['soajs.service.vm.group'] = idInfo[idInfo.indexOf('resourceGroups') + 1];
			}
			if (opts.vm.hardwareProfile && opts.vm.hardwareProfile.vmSize) {
				record.labels['soajs.service.vm.size'] = opts.vm.hardwareProfile.vmSize;
			}
			...
			record.tasks[0] = {};
			if (opts.vm.name) {
				record.tasks[0].id = opts.vm.name;
			}
			if (opts.vm.name) {
				record.tasks[0].name = opts.vm.name;
			}
			
			record.tasks[0].status = {};
			if (opts.vm.provisioningState) {
				record.tasks[0].status.state = opts.vm.provisioningState.toLowerCase();
			}
			if (opts.vm.provisioningState) {
				record.tasks[0].status.ts = new Date().getTime();
			}
			
			record.tasks[0].ref = {os: {}};
			if (opts.vm.storageProfile) {
				if (opts.vm.storageProfile.osDisk) {
					if (opts.vm.storageProfile.osDisk.osType) {
						record.tasks[0].ref.os.type = opts.vm.storageProfile.osDisk.osType;
					}
					if (opts.vm.storageProfile.osDisk.diskSizeGB) {
						record.tasks[0].ref.os.diskSizeGB = opts.vm.storageProfile.osDisk.diskSizeGB;
					}
				}
				if (opts.vm.storageProfile.imageReference) {
					record.tasks[0].ref.os.image = {};
					if (opts.vm.storageProfile.imageReference.publisher) {
						record.tasks[0].ref.os.image.prefix = opts.vm.storageProfile.imageReference.publisher;
					}
					if (opts.vm.storageProfile.imageReference.offer) {
						...
						if (opts.vm.storageProfile.imageReference.sku) {
							record.tasks[0].ref.os.image.name += "__" + opts.vm.storageProfile.imageReference.sku;
						}
					}
					if (opts.vm.storageProfile.imageReference.version) {
						record.tasks[0].ref.os.image.version = opts.vm.storageProfile.imageReference.version;
					}
				}
		...
		if (opts.loadBalancers && Array.isArray(opts.loadBalancers) && opts.loadBalancers.length > 0) {
			...
			opts.loadBalancers.forEach(function (oneLoadBalancer) {
				let loadBalancerRecord = helper.buildLoadBalancerRecord({
					loadBalancer: oneLoadBalancer,
					publicIpsList: opts.publicIpsList
				});
	...
	buildVmImagePublisherssRecord: function (opts) {
		...
		if (opts.imagePublisher) {
			if (opts.imagePublisher.name) {
				record.name = opts.imagePublisher.name;
			}
			if (opts.imagePublisher.id) {
				record.id = opts.imagePublisher.id;
			}
			if (opts.imagePublisher.location) {
				record.region = opts.imagePublisher.location;
			}
		}
	...
	buildVmImagePublishersOffersRecord: function (opts) {
		...
		if (opts.imageOffer) {
			if (opts.imageOffer.name) {
				record.name = opts.imageOffer.name;
			}
			if (opts.imageOffer.id) {
				record.id = opts.imageOffer.id;
			}
			if (opts.imageOffer.location) {
				record.region = opts.imageOffer.location;
			}
			if (opts.imageOffer.publisher) {
				record.publisher = opts.imageOffer.publisher;
			}
			if (opts.imageOffer.imageName) {
				record.imageName = opts.imageOffer.imageName;
			}
		}
	...
	buildVmImageVersionsRecord: function (opts) {
		...
		if (opts.imageVersion) {
			if (opts.imageVersion.name) {
				record.name = opts.imageVersion.name;
			}
			if (opts.imageVersion.id) {
				record.id = opts.imageVersion.id;
			}
			if (opts.imageVersion.location) {
				record.region = opts.imageVersion.location;
			}
			if (opts.imageVersion.publisher) {
				record.publisher = opts.imageVersion.publisher;
			}
			if (opts.imageVersion.imageName) {
				record.imageName = opts.imageVersion.imageName;
			}
		}
	...
	buildDiskRecord: function (opts) {
		...
		if (opts.disk) {
			if (opts.disk.name) {
				record.name = opts.disk.name;
			}
			if (opts.disk.id) {
				record.id = opts.disk.id;
			}
			if (opts.disk.location) {
				record.region = opts.disk.location;
			}
			if (opts.disk.diskSizeGB) {
				record.diskSizeGB = opts.disk.diskSizeGB;
			}
			record.type = (opts.disk.osType) ? 'os' : 'data';
			if (opts.disk.sku && opts.disk.sku.name) {
				record.storageType = opts.disk.sku.name;
			}
			if (opts.disk.timeCreated) {
				record.created = opts.disk.timeCreated;
			}
		}
		...
		async.auto({
			getLoadBalancers: function (callback) {
				networkClient.networkInterfaceLoadBalancers.list(resourceGroupName, networkInterfaceName, function (error, loadBalancers) {
					if (error) {
						opts.log.warn(`Unable to get load balancers for network interface ${networkInterfaceName}`);
					}
		...
		async.auto({
			...
			listNetworkInterfaces: function (callback) {
				networkClient.networkInterfaces.list(opts.group, function (error, networkInterfaces) {
					if (error) {
						opts.log.warn(`Unable to list network interfaces`);
					}
			...
			listSecurityGroups: function (callback) {
				networkClient.networkSecurityGroups.list(opts.group, function (error, securityGroups) {
					if (error) {
						opts.log.warn(`Unable to list security groups`);
					}
			...
			listPublicIps: function (callback) {
				networkClient.publicIPAddresses.list(opts.group, function (error, publicIps) {
					if (error) {
						opts.log.warn(`Unable to list public ip addresses`);
					}
			...
			listLoadBalancers: function (callback) {
				networkClient.loadBalancers.list(opts.group, function (error, loadBalancers) {
					if (error) {
						opts.log.warn(`Unable to list load balancers`);
					}
					return callback(null, loadBalancers || []);
				});
			}
	...
	buildResourceGroupRecord: function (opts) {
		...
		if (opts.resourceGroup) {
			if (opts.resourceGroup.id) {
				record.id = opts.resourceGroup.id;
			}
			if (opts.resourceGroup.name) {
				record.name = opts.resourceGroup.name;
			}
			if (opts.resourceGroup.location) {
				record.region = opts.resourceGroup.location;
			}
			if (opts.resourceGroup.tags) {
				record.labels = opts.resourceGroup.tags;
			}
		}
	...
	buildVmSizes: function (opts) {
		...
		if (opts.vmSize) {
			if (opts.vmSize.name) {
				record.name = opts.vmSize.name;
			}
			if (opts.vmSize.numberOfCores) {
				record.numberOfCores = opts.vmSize.numberOfCores;
			}
			if (opts.vmSize.osDiskSizeInMB) {
				record.osDiskSizeInMB = opts.vmSize.osDiskSizeInMB;
			}
			if (opts.vmSize.resourceDiskSizeInMB) {
				record.resourceDiskSizeInMB = opts.vmSize.resourceDiskSizeInMB;
			}
			if (opts.vmSize.memoryInMB) {
				record.memoryInMB = opts.vmSize.memoryInMB;
			}
			if (opts.vmSize.maxDataDiskCount) {
				record.maxDataDiskCount = opts.vmSize.maxDataDiskCount;
			}
	...
	buildNetworkRecord: function (opts) {
		...
		if (opts.network) {
			if (opts.network.name) {
				record.name = opts.network.name;
			}
			if (opts.network.id) {
				record.id = opts.network.id;
			}
			if (opts.network.location) {
				record.region = opts.network.location;
			}
	...
	buildLoadBalancerRecord: function (opts) {
		...
		if (opts.loadBalancer) {
			if (opts.loadBalancer.name) {
				record.name = opts.loadBalancer.name;
			}
			if (opts.loadBalancer.id) {
				record.id = opts.loadBalancer.id;
			}
			if (opts.loadBalancer.location) {
				record.region = opts.loadBalancer.location;
			}
	...
	buildSubnetRecord: function (opts) {
		...
		if (opts.subnet) {
			if (opts.subnet.name) {
				record.name = opts.subnet.name;
			}
			if (opts.subnet.id) {
				record.id = opts.subnet.id;
			}
			if (opts.subnet.location) {
				record.region = opts.subnet.location;
			}
			if (opts.subnet.addressPrefix) {
				record.address = opts.subnet.addressPrefix;
			}
		}
	...
	buildPublicIPRecord: function (opts) {
		...
		if (opts.publicIPAddress) {
			if (opts.publicIPAddress.name) {
				record.name = opts.publicIPAddress.name;
			}
			if (opts.publicIPAddress.id) {
				record.id = opts.publicIPAddress.id;
			}
			if (opts.publicIPAddress.location) {
				record.region = opts.publicIPAddress.location;
			}
			if (opts.publicIPAddress.ipAddress) {
				record.address = opts.publicIPAddress.ipAddress;
			}
			if (opts.publicIPAddress.publicIPAllocationMethod) {
				record.publicIPAllocationMethod = opts.publicIPAddress.publicIPAllocationMethod.toLowerCase();
			}
			if (opts.publicIPAddress.idleTimeoutInMinutes) {
				record.idleTimeout = opts.publicIPAddress.idleTimeoutInMinutes * 60;
			}
			if (opts.publicIPAddress.publicIPAddressVersion) {
				record.ipAddressVersion = opts.publicIPAddress.publicIPAddressVersion;
			}
			if (opts.publicIPAddress.tags) {
				record.labels = opts.publicIPAddress.tags;
			}
			if (opts.publicIPAddress.sku && opts.publicIPAddress.sku.name) {
				record.type = opts.publicIPAddress.sku.name.toLowerCase();
			}
	...
	buildSecurityGroupsRecord: function (opts) {
		...
		if (opts.networkSecurityGroups) {
			if (opts.networkSecurityGroups.name) {
				record.name = opts.networkSecurityGroups.name;
			}
			if (opts.networkSecurityGroups.id) {
				record.id = opts.networkSecurityGroups.id;
			}
			if (opts.networkSecurityGroups.location) {
				record.region = opts.networkSecurityGroups.location;
			}
			...
			if (opts.networkSecurityGroups.defaultSecurityRules && Array.isArray(opts.networkSecurityGroups.defaultSecurityRules) && opts.networkSecurityGroups.defaultSecurityRules.length > 0) {
				...
				defaultSecurityRules.map((oneSecurityRule) => {
					oneSecurityRule.readonly = true;
				});
				if (!record.ports) {
					record.ports = [];
				}
			...
			if (opts.networkSecurityGroups.tags) {
				record.labels = opts.networkSecurityGroups.tags;
			}
		}
		return record;
	},
	...
	getInfoFromAzureId: function (id, values) {
		...
		if (idInfo.length > 0) {
			for (let i = 0; i < idInfo.length; i++) {
				let match = values.find((oneValue) => {
					return oneValue.v === idInfo[i];
				});
