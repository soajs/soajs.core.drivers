		...
		ec2.describeKeyPairs({}, function (error, keyPairs) {
			...
			if (keyPairs && keyPairs.KeyPairs && Array.isArray(keyPairs.KeyPairs) && keyPairs.KeyPairs.length > 0) {
				...
				keyPairs.KeyPairs.forEach((oneKeyPair) => {
					let tempObj = {};
					if (oneKeyPair.KeyName) {
						tempObj.name = oneKeyPair.KeyName;
					}
					if (oneKeyPair.KeyFingerprint) {
						tempObj.fingerprint = oneKeyPair.KeyFingerprint;
					}
	...
	create: function (options, cb) {
		...
		ec2.createKeyPair(params, function (error, response) {
			if (error) {
				...
				if (response.KeyFingerprint) {
					keyPair.fingerprint = response.KeyFingerprint;
				}
				if (response.KeyName) {
					keyPair.name = response.KeyName;
				}
				keyPair.region = options.params.region;
				
				if (response.KeyMaterial) {
					keyPair.privateKey = response.KeyMaterial;
				}
