		...
		ec2.describeAddresses({}, function (error, ips) {
			...
			if (ips && ips.Addresses && Array.isArray(ips.Addresses) && ips.Addresses.length > 0) {
				...
				ips.Addresses.forEach((oneIP) => {
					let tempObj = {};
					if (oneIP.PublicIp) {
						tempObj.address = oneIP.PublicIp;
					}
					if (oneIP.AllocationId) {
						tempObj.id = oneIP.AllocationId;
					}
					if (oneIP.Domain) {
						tempObj.type = oneIP.Domain;
					}
					if (oneIP.InstanceId) {
						tempObj.instanceId = oneIP.InstanceId;
					}
					if (oneIP.PrivateIpAddress) {
						tempObj.privateAddress = oneIP.PrivateIpAddress;
					}
