...
const utils = {
	...
	"validateInput": function (soajs, opts, cb) {
		let myValidator = new soajs.validator.Validator();
		let schema;

		//create a copy of the inputs
		let inputsCopy = coreLibs.utils.cloneObj(opts.inputs);

		try {
			schema = require('../../infra/' + opts.infra + '/schemas/' + opts.extra + '.js')
		} catch (e) {
			return cb({
				code: 761,
		...
		if (opts.extra === 'group') delete inputsCopy.group;
