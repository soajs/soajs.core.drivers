...
const utils = {
	...
	"validateInputs": function (options, command, cb) {
		let myValidator = new options.soajs.validator.Validator();
		let schema, extra;
		...
		try {
			schema = require('../../infra/' + options.infra.name + '/schemas/' + extra + '.js');
		} catch (e) {
			return cb({
				code: 761,
		...
		let status = myValidator.validate(inputs, schema);

		options.soajs.log.debug(status);
