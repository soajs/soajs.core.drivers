...
const utils = {
	...
	"checkPorts": function (options, cb) {
		if (!options.params.catalog.recipe.deployOptions || !options.params.catalog.recipe.deployOptions.ports || options.params.catalog.recipe.deployOptions.ports.length === 0) {
			return cb();
		}
		...
		async.each(ports, (onePort, callback) => {
			...
			if (options.params.inputmaskData && options.params.inputmaskData.custom && options.params.inputmaskData.custom.ports && options.params.inputmaskData.custom.ports.length > 0) {
			...
			if (error) {
				if (error.wrongPort) {
					return cb({"code": 722, "message": errors[722]});
				}
				if (error.invalidPorts) {
					return cb({"code": 723, "message": errors[723]});
				}
			}
			else {
	...
	"checkVolumes": function (options, driver, volumeSchema, cb) {
		if (!options.params.catalog.recipe.deployOptions || !options.params.catalog.recipe.deployOptions.voluming || options.params.catalog.recipe.deployOptions.voluming.length === 0) {
			return cb();
		}
		...
		if (check.errors && check.errors.length > 0) {
			return cb({"code": 724, "message": errors[724]});
		}
		else {
			return cb();
		}
	...
	"getLatestSOAJSImageInfo": function (options, cb) {
		...
		request.get(opts, function (error, response, body) {
			...
			if (!options.params.data.labels) {
				options.params.data.labels = {};
			}
			options.params.imageInfo = new Date(body.last_updated).getTime().toString();
			return cb(null, true);
		});
	},
	...
	"checkControllers": function(options, getServiceHost, cb) {
		...
		if (options.param.catalog.recipe.buildOptions && options.param.catalog.recipe.buildOptions.env) {
			for (let env in options.param.catalog.recipe.buildOptions.env) {
				if (options.param.catalog.recipe.buildOptions.env[env].value === '$SOAJS_NX_CONTROLLER_NB') {
					getControllers = true;
				}
			}
		}
		...
		if (getControllers) {
			...
			getServiceHost(options, (error, controllerDomainName) =>{
				if (error && error.source === 'driver' && error.code === 661) {
					error.msg = errors[999];
					return cb(error);
				}
				else{
					options.params.data.variables.push('SOAJS_NX_CONTROLLER_IP_1=' + controllerDomainName);
					options.params.data.variables.push('SOAJS_NX_CONTROLLER_NB=1');
					options.params.data.variables.push('SOAJS_NX_CONTROLLER_PORT=' + options.soajs.registry.services.config.ports.controller);
					return cb();
				}
			});
		}
