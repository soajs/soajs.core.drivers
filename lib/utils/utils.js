...
const utils = {
	...
	"checkPorts": function (options, cb) {
		...
		else if (options.params.catalog && options.params.catalog.recipe && options.params.catalog.recipe.deployOptions.ports) {
	...
	"checkControllers": function (options, getServiceHost, cb) {
		...
		let envVariables = ['$SOAJS_NX_CONTROLLER_IP', '$SOAJS_NX_CONTROLLER_NB', '$SOAJS_NX_CONTROLLER_PORT', '$SOAJS_REGISTRY_API'];
		if (options.params.catalog.recipe.buildOptions && options.params.catalog.recipe.buildOptions.env) {
			for (let env in options.params.catalog.recipe.buildOptions.env) {
				if (envVariables.includes(options.params.catalog.recipe.buildOptions.env[env].value)) {
					getControllers = true;
				}
			}
		}
		...
		if (getControllers) {
			...
			controllerOptions.params = {
				...
				serviceName: options.soajs.registry.services.controller.name || "controller"
			};
			...
			getServiceHost(controllerOptions, (error, controllerDomainName) => {
				if (error && error.source === 'driver' && error.code === 661) {
					...
					options.params.data.variables['$SOAJS_REGISTRY_API'] = controllerDomainName + ":" + options.params.data.variables['$SOAJS_CONTROLLER_PORT_MAINTENANCE'];
					return cb();
				}
			});
		} else {
