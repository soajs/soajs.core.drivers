...
const engine = {
	...
	"constructServiceLabels" : function(options) {
		...
		let imageTs = new Date(options.params.imageInfo.last_updated).getTime().toString();
	...
	"constructDeployerParams": function (options) {
		...
		let serviceParams = {
			...
			"id": options.params.data.name.toLowerCase() || options.params.data.serviceName.toLowerCase(),
			"name": options.params.data.name.toLowerCase() || options.params.data.serviceName.toLowerCase(),
			"image": image,
