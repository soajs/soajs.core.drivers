...
const engine = {
	...
	"constructServiceLabels": function (options) {
		...
		if (options.params.catalog.recipe.deployOptions.labels) {
			for (let labelKey in options.params.catalog.recipe.deployOptions.labels) {
				if (options.params.catalog.recipe.deployOptions.labels.hasOwnProperty(labelKey)) {
		...
		if (options.params.catalog.recipe.deployOptions.image.shell) {
			options.params.data.labels['soajs.shell'] = 'shell' + options.params.catalog.recipe.deployOptions.image.shell;
		}
		else {
			options.params.data.labels['soajs.shell'] = 'shell' + '/bin/bash';
		}
		
		function generateSOAJSLabels() {
			let serviceLabel = options.env.toLowerCase() + "-" + options.params.data.name;
			if (options.params.data.name !== options.soajs.registry.services.controller.name || "controller") {
			...
			if (options.params.inputmaskData.custom && options.params.inputmaskData.custom.version) {
				...
				options.params.data.labels['soajs.service.version'] = "1";
			}
			...
			if (options.params.inputmaskData.gitSource) {
				...
				if (options.params.catalog && options.params.catalog.recipe && options.params.catalog.recipe.buildOptions && options.params.catalog.recipe.buildOptions.env) {
					for (let oneEnvName in options.params.catalog.recipe.buildOptions.env) {
						if (options.params.catalog.recipe.buildOptions.env.hasOwnProperty(oneEnvName)) {
	...
	"constructDeployerParams": function (options) {
		...
		let serviceParams = {
			...
			"memoryRequest": ((options.params.inputmaskData.deployConfig) ? options.params.inputmaskData.deployConfig.memoryRequest : null),
		...
		if (options.params.data.cronConfig) {
			if (options.params.data.cronConfig.cronTime) {
				serviceParams.schedule = options.params.data.cronConfig.cronTime;
			}
			if (options.params.data.cronConfig.concurrencyPolicy) {
				serviceParams.concurrencyPolicy = options.params.data.cronConfig.concurrencyPolicy;
				serviceParams.labels["soajs.concurrencyPolicy"] = options.params.data.cronConfig.concurrencyPolicy;
			}
		}
		...
		if (iPrefix) {
			serviceParams.labels['service.image.prefix'] = iPrefix;
		}
		if (iName) {
			serviceParams.labels['service.image.name'] = iName;
		}
		if (iTag) {
			serviceParams.labels['service.image.tag'] = iTag;
		}
		...
		for (let oneLabel in options.params.data.labels) {
			if (options.params.data.labels.hasOwnProperty(oneLabel)) {
	...
	"buildAvailableVariables": function (options) {
		...
		for (let i in options.params.data.variables) {
			if (options.params.data.variables.hasOwnProperty(i)) {
