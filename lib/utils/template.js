...
const engine = {
	...
	"constructServiceLabels" : function(options) {
		...
		if (options.params.inputmaskData.action === 'rebuild') {
			if (options.strategy === 'kubernetes' && options.params.catalog.recipe.deployOptions.image.pullPolicy !== 'Always') {
				...
				options.params.data.labels['service.image.ts'] = new Date(options.params.imageInfo.last_updated).getTime().toString();
			}
		}
		else {
			options.params.data.labels['service.image.ts'] = new Date(options.params.imageInfo.last_updated).getTime().toString();
		...
		function generateSOAJSLabels() {
			options.params.data.labels = {
				...
				"soajs.service.repo.name": "soajs_" + options.params.data.serviceName,
		...
		function generateNginxLabels() {
			...
			if (options.params.inputmaskData.custom && options.params.inputmaskData.custom.name) {
				...
				if (options.params.data.name === 'nginx') {
					options.params.data.name = options.env.toLowerCase() + "-nginx";
				}
		...
		function generateOtherLabels() {
			...
			if (options.params.inputmaskData.custom && options.params.inputmaskData.custom.name) {
				...
				if (options.params.inputmaskData.custom.resourceId) {
					...
					if (options.params.inputmaskData.custom.version) {
	...
	"constructDeployerParams": function (options) {
		...
		options.params.inputmaskData.deployConfig.replication.mode = utils.verifyReplicationMode(options);
		...
		let myImage = mapImage(); //[image, iPrefix, iName, iTag]
		let image = myImage[0], iPrefix = myImage[1], iName = myImage[2], iTag = myImage[3];
		
		if (options.strategy === 'docker' && options.params.data.serviceName !== 'controller') {
		...
		else if (options.strategy === 'kubernetes') {
			options.params.data.serviceName += (options.params.inputmaskData.custom.version) ? "-v" + options.params.inputmaskData.custom.version : "";
		}
		
		let serviceParams = {
			"env": options.env.toLowerCase(),
			"id": options.params.data.serviceName.toLowerCase(),
			"name": options.params.data.serviceName.toLowerCase(),
			"image": image,
		...
		if (options.strategy === 'kubernetes' && options.params.catalog.recipe && options.params.catalog.recipe.deployOptions && options.params.catalog.recipe.deployOptions.readinessProbe && Object.keys(options.params.catalog.recipe.deployOptions.readinessProbe).length > 0) {
		...
		//map computed envs
		serviceParams.variables = engine.computeCatalogEnvVars(options);
		
		return serviceParams;
		
		function mapImage(){
			let image = '', iPrefix, iName, iTag;
			...
			return [image, iPrefix, iName, iTag];
		}
	},
	 ...
	 * @returns {*}
	 */
	"computeCatalogEnvVars" : function (options, cb) {
		...
		async.concat(catalogEnvs, function (oneEnvName, callback) {
			...
			// if env variable is of type userInput, get value from request body, if not found see use default value
				...
				if (config.dynamicCatalogVariables.indexOf(oneEnv.value) !== -1) {
					...
					Object.keys(options.params.data.variables).forEach(function (oneVar) {
						if (oneVar.match(regex)) {
							result.push(oneEnvName + '_' + nCount++ + '=' + options.params.data.variables[oneVar]);
						}
					});
				}
				else {
					if (oneEnv.value && options.params.data.variables[oneEnv.value]) {
						result.push(oneEnvName + '=' + options.params.data.variables[oneEnv.value]);
					}
				}
			}
