...
const daemonsets = {
	...
	get(deployer, opts, cb) {
		async function main() {
			if (opts.daemonset) {
				return await deployer.apis.apps.v1.namespaces(opts.namespace).daemonsets(opts.daemonset).get({qs: opts.qs});
			} else {
	...
	put(deployer, opts, cb) {
		async function main() {
			return await deployer.apis.apps.v1.namespaces(opts.namespace).daemonsets(opts.deployment).post({body: opts.body, qs: opts.qs});
		}
