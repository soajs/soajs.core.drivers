...
const clusterrolebindings = {
	...
	get(deployer, opts, cb) {
		async function main() {
			return await deployer["rbac.authorization.k8s.io"].v1alpha1.clusterrolebindings(opts.name).get();
		}
	...
	delete(deployer, opts, cb) {
		async function main() {
			return await deployer["rbac.authorization.k8s.io"].v1alpha1.clusterrolebindings(opts.name).delete();
		}
