...
const deployments = {
	...
	get(deployer, opts, cb) {
		async function main() {
			if (opts.name) {
				return await deployer.apis.apps.v1beta1.namespaces(opts.namespace).deployments(opts.name).get({qs: opts.qs});
			} else {
				return await deployer.apis.apps.v1beta1.namespaces(opts.namespace).deployments.get({qs: opts.qs});
			}
		}
