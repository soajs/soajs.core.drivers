        ...
        lib.getDeployer(options, (error, deployer) => {
            utils.checkError(error, 520, cb, () => {
                ...
                utils.checkError(!validInput, 674, cb, () => {
	                ...
	                wrapper.autoscale.get(deployer, { name: options.params.id , namespace: namespace}, (error, hpa) => {
                ...
                utils.checkError(options.params.type !== 'deployment', 679, cb, () => {
                    utils.checkError(!validInput, 674, cb, () => {
                        ...
                        async.map(Object.keys(options.params.metrics), (oneMetric, callback) => {
                            ...
                            let namespace = lib.buildNameSpace(options);
                           wrapper.autoscale.post(deployer, { body: autoscaler, namespace: namespace}, (error) => {
	                ...
	                wrapper.autoscale.get(deployer,{ name: options.params.id , namespace: namespace, }, (error, hpa) => {
                        utils.checkError(error, 675, cb, () => {
                            ...
                            async.map(Object.keys(options.params.metrics), (oneMetric, callback) => {
	                            ...
	                            wrapper.autoscale.put(deployer,{ name: options.params.id, body: hpa, namespace: namespace }, (error) => {
    ...
    deleteAutoscaler (options, cb) {
        lib.getDeployer(options, (error, deployer) => {
            utils.checkError(error, 520, cb, () => {
                ...
                utils.checkError(!validInput, 674, cb, () => {
                    let namespace = lib.buildNameSpace(options);
	                wrapper.autoscale.delete(deployer,{ name: options.params.id, namespace: namespace}, (error) => {
