...
const engine = {
		...
		lib.getDeployer(options, (error, deployer) => {
			utils.checkError(error, 520, cb, () => {
				utils.checkError(!options.params || !options.params.id, 655, cb, () => {
					wrapper.node.get(deployer, {name: options.params.id}, (error, node) => {
	...
	removeNode(options, cb) {
		lib.getDeployer(options, (error, deployer) => {
			utils.checkError(error, 520, cb, () => {
				wrapper.node.delete(deployer, {name: options.params.id}, (error) => {
	...
	updateNode(options, cb) {
		...
		if (options && options.params) {
			if (options.params.availability === 'active') {
				unschedulable = false;
			} else if (options.params.availability === 'drain') {
				unschedulable = true;
			}
		}
		lib.getDeployer(options, (error, deployer) => {
			utils.checkError(error, 520, cb, () => {
				wrapper.node.get(deployer, {name: options.params.id}, (error, node) => {
					utils.checkError(error, 655, cb, () => {
						...
						wrapper.node.put({name: options.params.id, body: node}, (error) => {
