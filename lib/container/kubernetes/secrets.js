...
const engine = {
			...
			utils.checkError(error, 520, cb, () => {
				...
				utils.checkError(check, 570, cb, () => {
					...
					lib.checkNameSpace(deployer, options, cb, function (error, namespace) {
						wrapper.secret.post(deployer, {body: secret, namespace: namespace}, function (error, secret) {
		...
		lib.getDeployer(options, (error, deployer) => {
			utils.checkError(error, 520, cb, () => {
				if (options.params && options.params.namespace) {
					lib.checkNameSpace(deployer, options, cb, function (error, namespace) {
						wrapper.secret.get(deployer, {namespace: namespace}, function (error, secrets) {
					...
					wrapper.secret.get(deployer, {}, function (error, secrets) {
						utils.checkError(error, 562, cb, () => {
							return cb(null, lib.buildSecretRecord(secrets));
						});
					});
	...
	getSecret(options, cb) {
		lib.getDeployer(options, (error, deployer) => {
			utils.checkError(error, 520, cb, () => {
				lib.checkNameSpace(deployer, options, cb, function (error, namespace) {
					wrapper.secret.get(deployer, {name: options.params.name, namespace: namespace}, function (error, secret) {
