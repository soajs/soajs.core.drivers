...
const kubeLib = {
	...
	checkSecrets(options, secrets, cb) {
		if (options.soajs.inputmaskData.custom.secrets && Array.isArray(options.soajs.inputmaskData.custom.secrets) && options.soajs.inputmaskData.custom.secrets.length > 0) {
			if (secrets && secrets.length > 0) {
				let dockerServiceSecrets = [];
				let certificateSecret;
				options.soajs.inputmaskData.custom.secrets.forEach((oneInputSecret) => {
					let found = false;
