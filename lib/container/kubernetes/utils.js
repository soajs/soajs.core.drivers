...
const swagger = require('../../schemas/kubernetes/swagger/swagger.json');
...
const kubeLib = {
	...
	getDeployment(options, deployer, cb) {
		...
		wrapper.deployment.get(deployer, {name: options.params.id, namespace: namespace}, (error, deployment) => {
			if (error && error.code !== 404) {
				return cb(error);
			}
			
			if (deployment && deployment.metadata && deployment.metadata.name) {
			...
			type = 'daemonset';
			wrapper.daemonset.get(deployer, {name: options.params.id, namespace: namespace}, (error, daemonset) => {
				if (error && error.code !== 404) {
					return cb(error);
				}
				if (daemonset && daemonset.metadata && daemonset.metadata.name) {
					return cb(null, daemonset);
				}
				
				wrapper.cronjob.get(deployer, {name: options.params.id, namespace: namespace}, (error, cronJobs) => {
					if (error && error.code !== 404) {
						return cb(error);
					}
					if (cronJobs && cronJobs.metadata && cronJobs.metadata.name) {
						return cb(null, cronJobs);
					}
					return cb();
				});
			});
	...
	getDeployer(options, cb) {
		...
		if (options.kubeConfig) {
			...
			//add custom swagger file to support metrics.k8s.io
			kubeConfig = {
				config: {
					url: kubeURL,
					auth: {
			...
			if (options && options.deployerConfig && options.deployerConfig.auth && options.deployerConfig.auth.token) {
				kubeConfig.config.auth.bearer = options.deployerConfig.auth.token;
			}
	...
	buildDeploymentRecord(options, deployerObject) {
		...
		if (options.deployment) {
			...
			if (options.deployment.spec && options.deployment.spec.jobTemplate) {
				record.env = getEnvVariables(options.deployment.spec.jobTemplate.spec.template.spec);
				record.resources = getResources(options.deployment.spec.jobTemplate.spec.template.spec);
			} else {
			...
			if (options.deployment.spec.schedule) {
				record.schedule = options.deployment.spec.schedule;
			}
			if (options.deployment.spec.concurrencyPolicy) {
				record.concurrencyPolicy = options.deployment.spec.concurrencyPolicy;
			}
		...
		if (!publishedService) {
			delete record.ip;
			if (options.service && options.service.spec && options.service.spec.clusterIP){
				record.ip = options.service.spec.clusterIP;
			}
		}
		...
		function getPorts(service) {
			if (!service || !service.hasOwnProperty('spec') || !service.spec.hasOwnProperty('ports')) {
				return [];
			}
		...
		function getResources(podSpec) {
			...
			if (podSpec && podSpec.containers && podSpec.containers.length > 0 && podSpec.containers[0].resources) {
				if (podSpec.containers[0].resources.limits) {
					if (podSpec.containers[0].resources.limits.memory) {
						resources.limits.memory = podSpec.containers[0].resources.limits.memory;
					}
					if (podSpec.containers[0].resources.limits.cpu) {
						resources.limits.cpu = podSpec.containers[0].resources.limits.cpu;
					}
				}
				if (podSpec.containers[0].resources.requests) {
					resources.requests = {};
					if (podSpec.containers[0].resources.requests.memory) {
						resources.requests.memory = podSpec.containers[0].resources.requests.memory;
					}
					if (podSpec.containers[0].resources.requests.cpu) {
						resources.requests.cpu = podSpec.containers[0].resources.requests.cpu;
					}
				}
			}
		...
		function getVolumes(deployment) {
			...
			if (options.deployment.spec.jobTemplate) {
				template = options.deployment.spec.jobTemplate.spec.template;
			}
			if (template.spec.volumes && Array.isArray(template.spec.volumes)) {
				voluming.volumes = template.volumes;
			}
			if (template.spec.containers && Array.isArray(template.spec.containers) && template.spec.containers[0]) {
				if (template.spec.containers[0].volumeMounts && Array.isArray(template.spec.containers[0].volumeMounts)) {
					voluming.volumeMounts = template.spec.containers[0].volumeMounts;
				}
			}
	...
	buildPodRecord(options) {
		...
		if (options.pod.status.conditions && options.pod.status.conditions.length > 0) {
			options.pod.status.conditions.forEach((condition) => {
				if (condition.type.toLowerCase() === "ready") {
					record.status.ready = condition.status;
				}
			});
		}
		return record;
	...
	buildAutoscalerRecord(options) {
		...
		if (options.hpa) {
			if (options.hpa.spec) {
				if (options.hpa.spec.minReplicas) {
					record.replicas.min = options.hpa.spec.minReplicas;
				}
				if (options.hpa.spec.maxReplicas) {
					record.replicas.max = options.hpa.spec.maxReplicas;
				}
	...
	buildLabelSelector(options) {
		...
		if (!options || !options.matchLabels || Object.keys(options.matchLabels).length === 0) {
	...
	buildPVCRecord(options) {
		let response = [];
		if (options.items && options.items.length > 0) {
			options.items.forEach(function (onePVC) {
				response.push({
					name: onePVC.metadata.name,
					namespace: onePVC.metadata.namespace,
					uid: onePVC.metadata.uid,
					storage: onePVC.spec.resources.requests.storage,
					accessModes: onePVC.spec.accessModes,
					storageClassName: onePVC.spec.storageClassName,
					volumeMode: onePVC.spec.volumeMode
				});
			});
		}
		return response;
			...
			if (secrets && secrets.length > 0) {
				...
				async.eachSeries(options.params.inputmaskData.custom.secrets, function (oneInputSecret, callback) {
					...
					if (!found) {
						return callback({
							code: 726,
							message: errors[726].replace("%secret_name%", oneInputSecret.name)
						});
					} else {
