...
const engine = {
	...
	deployService(options, cb) {
							...
							if (service) {
								deployer.core.namespaces(namespace).services.post({body: service}, (error) => {
									utils.checkError(error, 525, cb, () => {
										return createDeployment(deployer, namespace, service);
									});
								});
							}
							else {
		...
		function createDeployment(deployer, namespace, payload) {
			options.soajs.log.debug("Deploying new Service:", payload);
	...
	redeployService(options, cb) {
		...
		if (['redeploy', 'rebuild'].indexOf(options.params.action) === -1) {
			return cb({
				source: 'driver',
				value: errorFile[501],
				code: 501,
		...
		function checkForExistingService(deployer, namespace, deployment, mCb) {
			let filter = {labelSelector: 'soajs.service.label=' + options.params.data.id};
