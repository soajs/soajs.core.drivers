...
const engine = {
	...
	redeployService(options, cb) {
		...
		if (!options || !options.params || ['redeploy', 'rebuild'].indexOf(options.params.action) === -1) {
			return cb({
				source: 'driver',
				value: errorFile[501],
				code: 501,
							...
							utils.checkError(error, (error && error.code) ? error.code : 400, cb, () => {
								...
								switch (options.params.action) {
									case 'redeploy':
										...
										break;
									case 'rebuild':
										if (deployment.spec.template.spec.containers[0].ports && deployment.spec.template.spec.containers[0].ports.length > 0) {
				...
				utils.checkError(error, 533, cb, () => {
					...
					if (servicesList && servicesList.items && servicesList.items.length > 0) {
						...
						if(service.spec.ports && service.spec.ports.length > 0){
							...
							deployer.core.namespaces(namespace).services.patch({
							...
							options.soajs.log.debug("Deleting Service:", service);
							deployer.core.namespaces(namespace).services.delete({name: service.metadata.name}, (error) => {
								utils.checkError(error, 673, cb, mCb);
							});
						}
					}
