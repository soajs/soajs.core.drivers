...
const engine = {
									...
									utils.checkError(error, 663, cb, () => {
										wrapper.cronjob.get(deployer, {qs: filter}, (error, cronList) => {
											utils.checkError(error, 663, cb, () => {
												...
												if (cronList && cronList.items) deployments = deployments.concat(cronList.items);
	...
	deployService(options, cb) {
		...
		function createDeployment(deployer, namespace, payload, cb) {
			...
			wrapper[options.params.type.toLowerCase()].post(deployer, {body: payload, namespace: namespace}, (error, deployment) => {
							...
							"generateTemplate": ['checkPorts', 'checkVolumes', 'checkSecrets', 'checkImage', 'getControllers', (info, mCb) => {
								...
								utils.checkError(!payload, 400, mCb, () => {
									...
									utils.checkError(!check, 653, mCb, () => {
										if (payload.kind.toLowerCase() === "cronjob"){
											if (!payload.spec.jobTemplate.spec.template.spec.containers[0].env) payload.spec.jobTemplate.spec.template.spec.containers[0].env = [];
										}
										else {
	...
	findService(options, cb) {
		lib.getDeployer(options, (error, deployer) => {
			utils.checkError(error, 520, cb, () => {
				...
				wrapper.node.get({}, (error, nodeList) => {
					utils.checkError(error, 521, cb, () => {
						wrapper.deployment.get({qs: filter, namespace: namespace}, (error, deploymentList) => {
							utils.checkError(error, 549, cb, () => {
								wrapper.daemonset.get({qs: filter, namespace: namespace}, (error, daemonsetList) => {
									utils.checkError(error, 663, cb, () => {
										wrapper.cronjob.get({
											qs: filter,
											namespace: namespace
											...
											utils.checkError(error, 663, cb, () => {
												...
												if (cronJobList && cronJobList.items && cronJobList.items.length > 0) deployments = deployments.concat(cronJobList.items);
				...
				"usingService": (mCb) => {
					wrapper.service.get(deployer,{
						...
						if (error && error.code !== 404) {
							...
							wrapper.service.delete(deployer, {service: service.items[0].metadata.name, namespace: namespace}, mCb);
						}
						else if (service.metadata){
							wrapper.service.delete(deployer, {service: service.metadata.name, namespace: namespace}, mCb);
						}
						 else return mCb();
					});
				},
			...
			utils.checkError(error, 520, cb, () => {
				...
				wrapper.deployment.get(deployer, {qs: filter}, (error, deploymentList) => {
					utils.checkError(error, 536, cb, () => {
						wrapper.daemonset.get(deployer, {qs: filter}, (error, daemonsetList) => {
							utils.checkError(error, 663, cb, () => {
								wrapper.daemonset.get(deployer, {qs: filter}, (error, cronJobList) => {
									utils.checkError(error, 663, cb, () => {
										let deployments = [];
										if (deploymentList && deploymentList.items && deploymentList.items.length > 0) deployments = deployments.concat(deploymentList.items);
										if (daemonsetList && daemonsetList.items && daemonsetList.items.length > 0) deployments = deployments.concat(daemonsetList.items);
