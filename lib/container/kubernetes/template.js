		...
		if (deployment.kind.toLowerCase() === "cronjob") {
			...
			if (options.params.newBuild.memoryLimit || options.params.newBuild.memoryRequest) {
				...
				if (options.params.newBuild.memoryRequest) {
					...
					if (!deployment.spec.jobTemplate.spec.template.spec.containers[0].resources.requests && deployment.spec.jobTemplate.spec.template.spec.containers[0].resources.limits ) {
			...
			if (options.params.newBuild.memoryLimit || options.params.newBuild.memoryRequest) {
				...
				if (options.params.newBuild.memoryRequest) {
					...
					if (!deployment.spec.template.spec.containers[0].resources.requests && deployment.spec.template.spec.containers[0].resources.limits) {
						deployment.spec.template.spec.containers[0].resources.requests = {
							memory : "128Mi"
						};
					}
