...
const engine = {
	...
	deployService(options, cb) {
		...
		options.params.data.labels['soajs.service.label'] = options.params.data.labels['soajs.env.code'] + "-";
		options.params.data.labels['soajs.service.label'] += options.params.data.labels['soajs.service.name'];
		if(options.params.data.labels['soajs.service.version']){
			options.params.data.labels['soajs.service.label'] += "-v" + options.params.data.labels['soajs.service.version'];
		}
		...
		if (options.params.data.ports && options.params.data.ports.length > 0) {
			...
			if (options.params.customName) {
				service.metadata.name = cleanLabel(options.params.data.name);
			}
			else {
				service.metadata.name = cleanLabel(options.params.data.name) + '-service';
			}
		...
		payload.metadata.name = cleanLabel(options.params.data.name);
	...
	rebuildService(deployment, options) {
		...
		options.params.newBuild.labels['soajs.service.label'] = options.params.newBuild.labels['soajs.env.code'] + "-";
		options.params.newBuild.labels['soajs.service.label'] += options.params.newBuild.labels['soajs.service.name'];
		if(options.params.newBuild.labels['soajs.service.version']){
			options.params.newBuild.labels['soajs.service.label'] += "-v" + options.params.newBuild.labels['soajs.service.version'];
		}
	...
	AddServicePorts(service, ports) {
		...
		if (ports && ports.length > 0) {
			ports.forEach((onePortEntry, portIndex) => {
				...
				//reassign the same node port
				if(service.spec.ports && service.spec.ports.length > 0){
					service.spec.ports.forEach((existingServicePort) => {
						if(existingServicePort.name === portConfig.name && existingServicePort.nodePort){
							portConfig.nodePort = existingServicePort.nodePort;
						}
					});
				}
