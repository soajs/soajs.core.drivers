	...
	deployService(options, cb) {
		...
		if (options.params.type.toLowerCase() === "cronjob"){
			...
			if (options.params.data.cpuLimit) {
				if (!payload.spec.jobTemplate.spec.template.spec.containers[0].resources) payload.spe.jobTemplate.specc.template.spec.containers[0].resources = {};
				if (!payload.spec.jobTemplate.spec.template.spec.containers[0].resources.limits) payload.spec.jobTemplate.spec.template.spec.containers[0].resources.limits = {};
	...
	rebuildService(deployment, options) {
		...
		if (deployment.kind.toLowerCase() === "cronjob") {
			...
			if (options.params.newBuild.memoryLimit) {
				if (!deployment.spec.jobTemplate.spec.template.spec.containers[0].resources) deployment.spec.jobTemplate.spec.template.spec.containers[0].resources = {};
				if (!deployment.spec.jobTemplate.spec.template.spec.containers[0].resources.limits) deployment.spec.jobTemplate.spec.template.spec.containers[0].resources.limits = {};
				deployment.spec.jobTemplate.spec.template.spec.containers[0].resources.limits.memory = options.params.newBuild.memoryLimit;
			}
