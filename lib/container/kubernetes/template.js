...
const engine = {
	...
	deployService(options, cb) {
		...
		if (options.params.data.ports && options.params.data.ports.length > 0) {
			...
			options.params.data.ports.forEach((onePortEntry, portIndex) => {
				let portConfig = {
					...
					name: onePortEntry.name.toLowerCase() || 'port' + portIndex,
		...
		if (options.params.data.readinessProbe) {
			payload.spec.template.spec.containers[0].readinessProbe = options.params.data.readinessProbe;
		}
		...
		if (ports && ports.length > 0) {
			ports.forEach((onePortEntry, portIndex) => {
				let portConfig = {
					...
					targetPort: onePortEntry.target,
					preserveClientIP: onePortEntry.preserveClientIP || false
				};
