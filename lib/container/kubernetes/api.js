				...
				utils.checkError(!validInput, 674, cb, () => {
					engine.loadResourceTemplates(options, (error, templates) => {
						utils.checkError(error, 681, cb, () => {
							...
							async.map(options.params.resources, (oneResource, callback) => {
								...
								wrapper[type][options.params.action](deployer, opts, (error, response) => {
									if (error && (error.code === 404 || error.code === 409)) {
										if (options.params.action === 'get' || options.params.action === 'post') {
											return callback(null, {});
										} else if (options.params.action === 'delete') {
