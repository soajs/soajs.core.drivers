...
const engine = {
	...
	deployService(options) {
		...
		if (options && options.driver && options.driver.split('.')[1] === 'local') {
			if (options.params.data.labels['soajs.service.name'] === options.soajs.registry.services.controller.name || "controller") {
	...
	redeployService(deployment) {
		...
		if (!update.TaskTemplate.ContainerSpec.Env) {
			update.TaskTemplate.ContainerSpec.Env = [];
		}
	...
	rebuildService(deployment, options) {
		...
		if (options.params.newBuild.memoryLimit) {
			if (!update.TaskTemplate.Resources) {
				update.TaskTemplate.Resources = {};
			}
			if (!update.TaskTemplate.Resources.Limits) {
				update.TaskTemplate.Resources.Limits = {};
			}
			if (!update.TaskTemplate.Resources.Limits.MemoryBytes) {
				update.TaskTemplate.Resources.Limits.MemoryBytes = {};
			}
