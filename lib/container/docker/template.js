...
const engine = {
	...
	deployService (options){
		...
		payload.Name = options.params.data.name;
		...
		if (options.params.data.readinessProbe && options.params.data.ports && Array.isArray(options.params.data.ports) && options.params.data.ports.length > 0) {
			if (options.params.data.readinessProbe.httpGet
				&& options.params.data.readinessProbe.httpGet.path
				&& options.params.data.readinessProbe.httpGet.port) {
				// if it matches this exactly
				// {
				// 		httpGet: {
				// 			path: "/heartbeat",
				// 			port: "maintenance"
				// 		}
				// 	}
				let port = options.params.data.ports.find((port) => { return port && port.name && port.name === options.params.data.readinessProbe.httpGet.port });
				if (options.params.data.readinessProbe.httpGet.path === "/heartbeat" && options.params.data.readinessProbe.httpGet.port === "maintenance") {
					if (port && port.target){
						payload.TaskTemplate.ContainerSpec.Healthcheck.Test = ["CMD-SHELL", `curl -f http://127.0.0.1:${port.target}${options.params.data.readinessProbe.httpGet.path}`];
						// auto increment the initial delay in seconds by 60 seconds, this should be removed in further releases when the readiness probe object becomes per technology (docker or kubernetes)
						if(options.params.data.readinessProbe.initialDelaySeconds) {
							options.params.data.readinessProbe.initialDelaySeconds += 60;
						}
						payload.TaskTemplate.ContainerSpec.Healthcheck.StartPeriod = options.params.data.readinessProbe.initialDelaySeconds ? options.params.data.readinessProbe.initialDelaySeconds * 1000000000 : 60000000000; //nanoseconds;
						payload.TaskTemplate.ContainerSpec.Healthcheck.Retries = options.params.data.readinessProbe.failureThreshold ? options.params.data.readinessProbe.failureThreshold : 3; //nanoseconds;
						payload.TaskTemplate.ContainerSpec.Healthcheck.Timeout = options.params.data.readinessProbe.timeoutSeconds ? options.params.data.readinessProbe.timeoutSeconds * 1000000000 : 2000000000; //nanoseconds;
						payload.TaskTemplate.ContainerSpec.Healthcheck.Interval = options.params.data.readinessProbe.periodSeconds ? options.params.data.readinessProbe.periodSeconds * 1000000000 : 5000000000; //nanoseconds;
					}
				}
				//else do nothing
			}
			else if (options.params.data.readinessProbe.docker && options.params.data.readinessProbe.docker.Test){
	...
	rebuildService (deployment, options) {
		...
		if (options.params.newBuild.readinessProbe && options.params.newBuild.ports && Array.isArray(options.params.newBuild.ports) && options.params.newBuild.ports.length > 0) {
			if (options.params.newBuild.readinessProbe.httpGet
				&& options.params.newBuild.readinessProbe.httpGet.path
				&& options.params.newBuild.readinessProbe.httpGet.port) {
				// if it matches this exactly
				// {
				// 		httpGet: {
				// 			path: "/heartbeat",
				...
				// 		}
				// 	}
				let port = options.params.data.ports.find((port) => { return port && port.name && port.name === options.params.newBuild.readinessProbe.httpGet.port });
				if (options.params.newBuild.readinessProbe.httpGet.path === "/heartbeat" && options.params.newBuild.readinessProbe.httpGet.port === "maintenance") {
					if (port && port.target){
						update.TaskTemplate.ContainerSpec.Healthcheck.Test = ["CMD-SHELL", `curl -f http://127.0.0.1:${port.target}${options.params.newBuild.readinessProbe.httpGet.path}`];
						// auto increment the initial delay in seconds by 60 seconds, this should be removed in further releases when the readiness probe object becomes per technology (docker or kubernetes)
						if(options.params.newBuild.readinessProbe.initialDelaySeconds) {
							options.params.newBuild.readinessProbe.initialDelaySeconds += 60;
						}
						update.TaskTemplate.ContainerSpec.Healthcheck.StartPeriod = options.params.newBuild.readinessProbe.initialDelaySeconds ? options.params.newBuild.readinessProbe.initialDelaySeconds * 1000000000 : 60000000000; //nanoseconds;
						update.TaskTemplate.ContainerSpec.Healthcheck.Retries = options.params.newBuild.readinessProbe.failureThreshold ? options.params.newBuild.readinessProbe.failureThreshold : 3; //nanoseconds;
						update.TaskTemplate.ContainerSpec.Healthcheck.Timeout = options.params.newBuild.readinessProbe.timeoutSeconds ? options.params.newBuild.readinessProbe.timeoutSeconds * 1000000000 : 2000000000; //nanoseconds;
						update.TaskTemplate.ContainerSpec.Healthcheck.Interval = options.params.newBuild.readinessProbe.periodSeconds ? options.params.newBuild.readinessProbe.periodSeconds * 1000000000 : 5000000000; //nanoseconds;
					}
				}
				//else do nothing
			}
			else if (options.params.newBuild.readinessProbe.docker && options.params.newBuild.readinessProbe.docker.Test){
				// use what the user supplies you with
