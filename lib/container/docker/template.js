...
const engine = {
			...
			if (options.params.data.readinessProbe.httpGet
				...
				if (options.params.data.readinessProbe.httpGet.path === "/heartbeat" && options.params.data.readinessProbe.httpGet.port === "maintenance") {
					if (port && port.target){
						...
						payload.TaskTemplate.ContainerSpec.Healthcheck.StartPeriod = options.params.data.readinessProbe.initialDelaySeconds ? options.params.data.readinessProbe.initialDelaySeconds * 1000000000 : 15000000000; //nanoseconds;
				...
				payload.TaskTemplate.Healthcheck = options.params.readinessProbe.docker;
			}
		}
		return payload;
	},
	...
	rebuildService (deployment, options) {
		...
		if (options.params.newBuild.readinessProbe && options.params.data.ports && Array.isArray(options.params.newBuild.ports) && options.params.newBuild.ports.length > 0) {
			if (options.params.newBuild.httpGet
				&& options.params.newBuild.httpGet.path
				&& options.params.newBuild.httpGet.port) {
				// if it matches this exactly
				// {
				// 		httpGet: {
				// 			path: "/heartbeat",
				...
				// 		}
				// 	}
				let port = options.params.data.ports.find((port) => { return port && port.name && port.name === options.params.newBuild.httpGet.port });
				if (options.params.newBuild.httpGet.path === "/heartbeat" && options.params.newBuild.httpGet.port === "maintenance") {
					if (port && port.target){
						update.TaskTemplate.ContainerSpec.Healthcheck.Test = ["CMD-SHELL", `curl -f http://127.0.0.1:${port.target}${options.params.newBuild.httpGet.path}`];
						update.TaskTemplate.ContainerSpec.Healthcheck.StartPeriod = options.params.newBuild.initialDelaySeconds ? options.params.newBuild.initialDelaySeconds * 1000000000 : 5000000000; //nanoseconds;
						update.TaskTemplate.ContainerSpec.Healthcheck.Retries = options.params.newBuild.failureThreshold ? options.params.newBuild.failureThreshold : 3; //nanoseconds;
						update.TaskTemplate.ContainerSpec.Healthcheck.Timeout = options.params.newBuild.timeoutSeconds ? options.params.newBuild.timeoutSeconds * 1000000000 : 2000000000; //nanoseconds;
						update.TaskTemplate.ContainerSpec.Healthcheck.Interval = options.params.newBuild.periodSeconds ? options.params.newBuild.periodSeconds * 1000000000 : 5000000000; //nanoseconds;
					}
				}
				//else do nothing
			}
			else if (options.params.newBuild.docker && options.params.data.readinessProbe.docker.Test){
				// use what the user supplies you with
				update.TaskTemplate.Healthcheck = options.params.readinessProbe.docker;
			}
		}
		update.TaskTemplate.ForceUpdate = 1;
