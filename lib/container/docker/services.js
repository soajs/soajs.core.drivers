				...
				async.auto({
					...
					"generateTemplate": ['checkPorts', 'checkVolumes', 'checkSecrets', 'checkImage', 'getControllers', (info, mCb) => {
						...
						switch (options.params.action) {
							case 'redeploy':
								payload = template.redeployService(service, options);
								break;
							case 'rebuild':
								payload = template.rebuildService(service, options);
								break;
							default:
								payload = null;
								break;
					...
					utils.checkError(error, (error && error.code) ? error.code : 400, cb, () => {
						options.soajs.log.debug("Updating Service:", result.generateTemplate);
