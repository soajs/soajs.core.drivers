...
const helper = {
	...
	backup: function (operation, options, data, cb) {
		...
		fs.mkdir(config.terraform.backupLocation, function (error) {
			...
			if (data.templateOutput && (data.templateOutput.output || data.templateOutput.error)) {
				let logs = '';
				if (data.templateOutput.output) {
					logs = logs.concat(data.templateOutput.output);
				}
				if (data.templateOutput.error) {
					logs = logs.concat(data.templateOutput.error);
				}
				zip.file('output.log', logs);
			}
			
			// zip file name: infraProviderName-layerName-operation-timestamp.zip
			if (options.infra && options.infra.name) {
				zipFileName += `${options.infra.name}-`;
			}
			if (options.params && options.params.layerName) {
				zipFileName += `${options.params.layerName}-`;
			}
			zipFileName += `${operation}-${new Date().getTime()}.zip`;
			zip.writeToFile(config.terraform.backupLocation + zipFileName, (err, res) => {
				console.log(err, res);
			});
			return cb();
		});
	...
	parseErrorOutput: function (options) {
		if (!options.excludedPart) {
			options.excludedPart = config.terraform.errorExcludedMessage;
		}
