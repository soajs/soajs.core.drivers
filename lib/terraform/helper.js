...
const helper = {
	 ...
	 * @return {void}
     */
    backup: function(operation, options, result, cb) {
        ...
        fs.mkdir(config.terraform.backupLocation, function(error) {
            ...
            if(result.renderTemplate && result.renderTemplate.render) {
                zip.file('template.tf', result.renderTemplate.render);
            }
            if(result.applyTemplate && (result.applyTemplate.output || result.applyTemplate.error)) {
                let logs = '';
                if(result.applyTemplate.output) logs = logs.concat(result.applyTemplate.output);
                if(result.applyTemplate.error) logs = logs.concat(result.applyTemplate.error);
